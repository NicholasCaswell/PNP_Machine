...
char pinTX = PB1; //not erroneous in compilation
...
// write null terminated string NOT NEEDED
...
float getDistanceHCSR04()
{
	...
	cli();
	// enable interrupt
	PCICR |= (1 << PCIE0); //GIMSK original register
	// enable pin
	PCMSK0 |= (1 << PCINT0); //could be reused
	sei();
